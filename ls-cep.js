; (function (window, document, commonjs) {
    ...
    function getAddress(options) {
        var cep = options.cep || '',
            source = options.source || ls_cep.sources.postmon,
    ...
    function setUpMap(areaId, mapId) {
        _apiKey = _apiKey || localStorage.getItem('ls-cep-api-key');

        if (!_apiKey) throw 'Please, set up the google api key using the "setUpApiKey" function.'
        ...
        iframe.src = _url.replace('{API_KEY}', _apiKey).replace('{CEP}', '15057-000');
    ...
    function setMapCep(mapId, cep) {
        if(!ls_cep.isValid(cep)) throw 'You must inform a valid cep format. (Ex: 99999-999 or 99999999)';
        ...
        var newUrl = _url.replace('{API_KEY}', _apiKey).replace('{CEP}', cep),
            iframe = document.getElementById(mapId);
    ...
    var ls_cep = {
        ...
        setUpMap: function (areaId, mapId) {
            return setUpMap(areaId, mapId);
        },
        setMapCep: function (mapId, cep) {
            setMapCep(mapId, cep);
        },
        setUpApiKey: function (apiKey, persist) {
            _apiKey = apiKey;

            if (!!persist) {
                !!localStorage && localStorage.setItem('ls-cep-api-key', _apiKey);
            } else {
